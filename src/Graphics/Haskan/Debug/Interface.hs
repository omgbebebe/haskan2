...
debugEventToAction (KeyPress keyName pressed) =
  case Text.toLower keyName of
    ...
    "f3" -> Just (ToggleWireframe, pressed, False)
    "f4" -> Just (DebugMode 3, pressed, False)
    "f5" -> Just (DebugMode 4, pressed, False)
    "f6" -> Just (DebugMode 5, pressed, False)
    "f7" -> Just (DebugMode 6, pressed, False)
    "f8" -> Just (DebugMode 7, pressed, False)
    "f9" -> Just (DebugMode 0, pressed, False)
