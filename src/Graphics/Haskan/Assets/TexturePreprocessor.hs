...
data TextureConfig = TextureConfig
  { -- | Desired width (Nothing = keep original, rounded up to PoT)
    tcTargetWidth :: !(Maybe Int),
    -- | Desired height (Nothing = keep original, rounded up to PoT)
    tcTargetHeight :: !(Maybe Int),
    -- | Resize to next power of two
    tcForcePowerOfTwo :: !Bool,
    -- | Generate mipmaps (placeholder)
    tcGenerateMips :: !Bool,
    -- | Output pixel format
    tcTargetFormat :: !TextureFormat
  }
  deriving (Eq, Show)
