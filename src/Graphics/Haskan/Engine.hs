...
mainLoop meshName EngineConfig {..} = do
  ...
  tvPhysicsPendingSpecs <- liftIO $ STM.newTVarIO []
  ...
  liftIO $ forkIOWithHandler "renderLoop" renderLoopFinished $ runManaged $ renderLoop window physicalDevice surface inst layers targetRenderFPS gameState renderLoopFinished renderLoopReady controlChannel meshName uvCheckMode envMapDir cloudTestMode proceduralSkyEnabled
