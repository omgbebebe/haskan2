...
import Foreign (Ptr, alloca, allocaArray, copyBytes, poke, sizeOf, plusPtr)
...
data SpecEntry = SpecEntry
  { seConstantID :: !Word32       -- ^ SpecId decoration in SPIR-V
  , seValue      :: !ByteString   -- ^ Raw value bytes (host-endian, sized to type)
  }
