...
managedCloudNoiseMipGenComputeDescriptorPool :: (MonadManaged m) => Vulkan.VkDevice -> m Vulkan.VkDescriptorPool
...
createCloudNoiseMipGenComputeDescriptorPool :: (MonadIO m) => Vulkan.VkDevice -> m Vulkan.VkDescriptorPool
createCloudNoiseMipGenComputeDescriptorPool dev = do
  let storageImagePoolSize =
        Vulkan.createVk
          ( set @"type" Vulkan.VK_DESCRIPTOR_TYPE_STORAGE_IMAGE
              &* set @"descriptorCount" 8
          )
      uboPoolSize =
        Vulkan.createVk
          ( set @"type" Vulkan.VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER
