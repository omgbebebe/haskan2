...
import Linear (V2 (..), V3 (..), V4 (..), cross, normalize, (^*), (^+^), (^-^), dot)
...
computeTangents ::
  Vector (V3 Float) ->    -- ^ positions
  Vector (V3 Float) ->    -- ^ normals
  Vector (V2 Float) ->    -- ^ UVs
  Vector Word32 ->         -- ^ indices (triangles)
  Vector (V4 Float)         -- ^ tangents (xyz = direction, w = handedness)
      ...
      finalize i =
        let V3 nx ny nz = normalize (normals ! i)
            ...
            V3 bsx bsy bsz = if bitanSum == zero then bitanRef else normalize bitanSum
              where bitanRef = V3 bitanRefX bitanRefY bitanRefZ
