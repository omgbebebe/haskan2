...
data CDLODConfig = CDLODConfig
  { -- | Vertices per patch edge (8 for 8×8)
    ccPatchSize :: !Int,
    -- | Maximum LOD level (0 = finest)
    ccMaxLOD :: !Int,
    -- | Total world size in meters
    ccWorldSize :: !Float,
    -- | Fraction of node edge that morphs (e.g. 0.2)
    ccMorphZoneRatio :: !Float,
    -- | Distance multiplier per LOD level
    ccLODDistanceRatio :: !Float
  }
  deriving (Show)
...
data TerrainNode = TerrainNode
  { -- | Node origin (x, z) in world space
    tnWorldOffset :: !(V2 Float),
    -- | Node edge length in world units
    tnWorldSize :: !Float,
    -- | LOD level (0 = finest)
    tnLOD :: !Int,
    -- | Texture array layer for heightmap
    tnHeightmapLayer :: !Word32,
    -- | Texture array layer for climate
    tnClimateLayer :: !Word32,
    -- | Distance where morph begins
    tnMorphStart :: !Float
  }
  deriving (Show)
...
buildCDLODTree cfg@CDLODConfig {..} =
  CDLODTree
    { ctConfig = cfg,
...
data NodeSelection = NodeSelection
  { -- | Visible leaf nodes at appropriate LOD
    nsNodes :: [TerrainNode],
    -- | Maximum LOD used
    nsMaxLOD :: !Int
  }
