...
adaptMaterial Material.Material{..} = Material
  { materialAlphaCutoff = alphaCutoff,
    ...
    materialNormalScale = maybe 1.0 (\ti -> case TextureInfo.subtype ti of Material.MaterialNormal s -> s) normalTexture,
    materialOcclusionTexture = adaptTextureInfo <$> occlusionTexture,
    materialOcclusionStrength = maybe 1.0 (\ti -> case TextureInfo.subtype ti of Material.MaterialOcclusion o -> o) occlusionTexture
  }
...
adaptPbrMetallicRoughness PbrMetallicRoughness.PbrMetallicRoughness{..}
  = PbrMetallicRoughness
    { pbrBaseColorFactor = toV4 baseColorFactor,
      ...
      pbrMetallicRoughnessTexture = adaptTextureInfo <$> metallicRoughnessTexture
    }
