...
-- | Decode vertex indices, respecting the accessor's componentType
vertexIndices :: GlTF -> Vector GltfBuffer -> AccessorIx -> Vector Word32
vertexIndices gltf buffers' accessorId =
  case (bufferAccessor gltf buffers' accessorId, accessors gltf) of
    (Nothing, _) -> mempty
    (Just _, Nothing) -> readBufferWithGet getIndices gltf buffers' accessorId
    (Just _, Just accs) ->
      case lookupAccessor accessorId accs of
        Nothing -> readBufferWithGet getIndices gltf buffers' accessorId
        Just accessor ->
          let Accessor{componentType = componentType'} = accessor
          in case componentType' of
            UNSIGNED_BYTE  ->
              Vector.map fromIntegral (readBufferWithGet (getScalar getUnsignedByte) gltf buffers' accessorId)
            UNSIGNED_SHORT ->
              Vector.map fromIntegral (readBufferWithGet (getScalar getUnsignedShort) gltf buffers' accessorId)
            UNSIGNED_INT   ->
              readBufferWithGet (getScalar getUnsignedInt) gltf buffers' accessorId
            _ -> error $ "Unsupported index componentType: " <> show (unComponentType componentType')
